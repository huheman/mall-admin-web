<template>
  <div style="margin-top: 50px">
    <el-form :model="value" :rules="rules" ref="productAttrForm" label-width="120px" class="form-inner-container"
      size="small">
      ...
      <el-form-item label="属性类型：">
        <el-select v-model="value.productAttributeCategoryId" placeholder="请选择属性类型" disabled
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      'value.productCategoryId': function(newVal) {
        if(newVal){
          let c = this.productCateOptions.filter(c=>{
            return c.value == this.selectProductCateValue[0]
          })[0].children.filter(c=>{
            return c.value == this.selectProductCateValue[1]
          })[0]
          this.handleProductAttrChange(c.attrId)
        }
  ...
  methods: {
          ...
          if (list[i].children != null && list[i].children.length > 0) {
            for (let j = 0; j < list[i].children.length; j++) {
              children.push({
                ...
                attrId: list[i].children[j].productCount
              });
            }
          }
    ...
    handleProductAttrChange(value) {
      this.value.productAttributeCategoryId=value
